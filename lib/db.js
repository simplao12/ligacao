...
export async function deletePlaylistById(code, playlistId) {
  ...
  try {
    ...
    console.error('[deletePlaylistById] erro — rollback executado:', error);
    throw error;
  } finally {
    client.release();
  }
...
export async function deletePlaylist(code) {
  ...
  try {
    ...
    console.log(`[deletePlaylist] ${del.rowCount} playlist(s) deletada(s) para código: ${code}`);

    const upd = await client.query(
      'UPDATE devices SET activated = false WHERE code = $1 RETURNING id',
      [code]
    );
    console.log(`[deletePlaylist] ${upd.rowCount} device(s) desativado(s) para código: ${code}`);

    await client.query('COMMIT');
    return true;
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('[deletePlaylist] erro — rollback executado:', error);
    throw error;
  } finally {
    client.release();
  }
