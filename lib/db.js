/**
 * Database layer — Vercel Postgres + fallback em-memória
 *
 * Em produção: usa Vercel Postgres (via DATABASE_URL)
 * Em dev: usa Map em memória (sem deps externas)
 */
...
export async function registerDevice(deviceId) {
  ...
  try {
    ...
    await sql`
      INSERT INTO devices (code, device_id, activated, created_at, last_seen)
...
export async function setPlaylist(code, playlistData) {
  ...
  try {
    ...
    await sql`
      DELETE FROM playlists WHERE device_code = $1
    `;

    // Insere nova playlist
    await sql`
      INSERT INTO playlists (device_code, kind, name, host, username, password, url, epg_url)
    ...
    await sql`
      UPDATE devices
      SET activated = true, last_seen = NOW()
      WHERE code = $1
    `;
...
export async function deletePlaylist(code) {
  ...
  try {
    ...
    await sql`
      UPDATE devices SET activated = false WHERE code = $1
    `;
